...
import PPage from "@/components/Page.vue";
...
export default class Home extends Vue {
  private get swiper() {
    return this.$refs["main-view"].swiper;
  }
  private repos: any[] = [];
  ...
  @Watch("animating")
  private onAnimating(val, oldVal) {
    if (oldVal) return;
    if (val) {
      const that = this;
  ...
  private setAnimating(val) {
    this.animating = val;
  }
  @Watch("swipeState")
  private onStateChanged(val, oldVal) {
    ...
    // console.log(path)
    this.$refs["color-box"].style['clip-path'] = path;
